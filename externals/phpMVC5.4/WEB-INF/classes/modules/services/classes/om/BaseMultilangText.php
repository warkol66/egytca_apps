...
abstract class BaseMultilangText extends BaseObject implements Persistent
{
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = MultilangTextQuery::create()
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(MultilangTextPeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`ID`';
        }
        if ($this->isColumnModified(MultilangTextPeer::MODULENAME)) {
            $modifiedColumns[':p' . $index++]  = '`MODULENAME`';
        }
        if ($this->isColumnModified(MultilangTextPeer::LANGUAGECODE)) {
            $modifiedColumns[':p' . $index++]  = '`LANGUAGECODE`';
        }
        if ($this->isColumnModified(MultilangTextPeer::TEXT)) {
            $modifiedColumns[':p' . $index++]  = '`TEXT`';
        }

        $sql = sprintf(
            'INSERT INTO `multilang_text` (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case '`MODULENAME`':
                        ...
                        break;
                    case '`LANGUAGECODE`':
                        $stmt->bindValue($identifier, $this->languagecode, PDO::PARAM_STR);
                        break;
                    case '`TEXT`':
     ...
     *
     * @return string The value of the 'text' column
     */
