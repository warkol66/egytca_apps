...
class ChatServer
{
  function prepare($method='', $arguments = array()) {

    $userId = null;
    $statelessName = 'statelessCMS';

    if ($GLOBALS['fc_config']['combineCMS'] && !isset($GLOBALS['fc_config']['statelessCMS'])) {
    ...
    if (!$GLOBALS['fc_config']['currentCMS'] || !$GLOBALS['fc_config']['cms']) {
  ...
  { // changed on 090706 for chat instances
    ...
    //try to login using current cms
    $GLOBALS['fc_config']['cms'] = $GLOBALS['fc_config']['currentCMS'];
    ...
    else
    {
      ...
      if (!$id && $GLOBALS['fc_config']['combineCMS']) {
        $GLOBALS['fc_config']['cms'] = $GLOBALS['fc_config']['statelessCMS'];
        $pref = ($GLOBALS['fc_config']['guestPrefix']) ? ('('.$GLOBALS['fc_config']['guestPrefix'].') ') : '';
        $id = $GLOBALS['fc_config']['cms']->login(addslashes($pref.$login), $password, $session_inst);
      }
  ...
  {
    ChatServer::prepare('getUser', array($userid));
  ...
  {
    ChatServer::prepare('getGender', array($userid));
    if($userid <= SPY_USERID)
    {
      return '';
    }
  ...
  {
    ChatServer::prepare('getPhoto', array($userid));
  ...
  {
    ChatServer::prepare('getUserProfile', array($userid));
