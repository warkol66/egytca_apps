...
abstract class BaseModuleLabelQuery extends ModelCriteria
{
     ...
     * @param     ModuleLabelQuery|Criteria $criteria Optional Criteria to build the query from
     *
     ...
     *
     * @return   ModuleLabel|ModuleLabel[]|mixed the result, formatted by the current formatter
     */
    public function findPk($key, $con = null)
    {
        ...
        if ((null !== ($obj = ModuleLabelPeer::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1]))))) && !$this->formatter) {
     ...
     *
     * @return   ModuleLabel A model object, or null if the key is not found
     * @throws   PropelException
     */
    protected function findPkSimple($key, $con)
    {
        $sql = 'SELECT `ID`, `NAME`, `LABEL`, `DESCRIPTION`, `LANGUAGE` FROM `modules_label` WHERE `ID` = :p0 AND `NAME` = :p1';
        try {
        ...
        if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
            $obj = new ModuleLabel();
            $obj->hydrate($row);
            ModuleLabelPeer::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
        }
        $stmt->closeCursor();
     ...
     * @param     PropelPDO $con an optional connection object
     *
     * @return PropelObjectCollection|ModuleLabel[]|mixed the list of results, formatted by the current formatter
     */
