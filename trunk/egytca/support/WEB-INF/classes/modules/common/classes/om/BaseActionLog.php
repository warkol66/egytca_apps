...
abstract class BaseActionLog extends BaseObject implements Persistent
{
     ...
     * Mensaje resultado de la accion
     * @return string
     */
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            return $startcol + 7; // 7 = ActionLogPeer::NUM_HYDRATE_COLUMNS.
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = ActionLogQuery::create()
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(ActionLogPeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`ID`';
        }
        if ($this->isColumnModified(ActionLogPeer::USEROBJECTTYPE)) {
            $modifiedColumns[':p' . $index++]  = '`USEROBJECTTYPE`';
        }
        if ($this->isColumnModified(ActionLogPeer::USEROBJECTID)) {
            $modifiedColumns[':p' . $index++]  = '`USEROBJECTID`';
        }
        if ($this->isColumnModified(ActionLogPeer::DATETIME)) {
            $modifiedColumns[':p' . $index++]  = '`DATETIME`';
        }
        if ($this->isColumnModified(ActionLogPeer::ACTION)) {
            $modifiedColumns[':p' . $index++]  = '`ACTION`';
        }
        if ($this->isColumnModified(ActionLogPeer::MESSAGE)) {
            $modifiedColumns[':p' . $index++]  = '`MESSAGE`';
        }
        if ($this->isColumnModified(ActionLogPeer::FORWARD)) {
            $modifiedColumns[':p' . $index++]  = '`FORWARD`';
        }

        $sql = sprintf(
            'INSERT INTO `actionLogs_log` (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case '`ID`':
                        ...
                        break;
                    case '`USEROBJECTTYPE`':
                        $stmt->bindValue($identifier, $this->userobjecttype, PDO::PARAM_STR);
                        break;
                    case '`USEROBJECTID`':
                        $stmt->bindValue($identifier, $this->userobjectid, PDO::PARAM_INT);
                        break;
                    case '`DATETIME`':
                    ...
                    case '`MESSAGE`':
                        ...
                        break;
                    case '`FORWARD`':
                        $stmt->bindValue($identifier, $this->forward, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    {
        if (isset($alreadyDumpedObjects['ActionLog'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['ActionLog'][$this->getPrimaryKey()] = true;
        ...
        if ($includeForeignObjects) {
            if (null !== $this->aSecurityAction) {
                $result['SecurityAction'] = $this->aSecurityAction->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
        }
    ...
    {
        return (string) $this->exportTo(ActionLogPeer::DEFAULT_STRING_FORMAT);
    }
