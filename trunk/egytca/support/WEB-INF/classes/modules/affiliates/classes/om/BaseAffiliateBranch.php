...
abstract class BaseAffiliateBranch extends BaseObject implements Persistent
{
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = AffiliateBranchQuery::create()
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(AffiliateBranchPeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`ID`';
        }
        if ($this->isColumnModified(AffiliateBranchPeer::AFFILIATEID)) {
            $modifiedColumns[':p' . $index++]  = '`AFFILIATEID`';
        }
        if ($this->isColumnModified(AffiliateBranchPeer::NUMBER)) {
            $modifiedColumns[':p' . $index++]  = '`NUMBER`';
        }
        if ($this->isColumnModified(AffiliateBranchPeer::CODE)) {
            $modifiedColumns[':p' . $index++]  = '`CODE`';
        }
        if ($this->isColumnModified(AffiliateBranchPeer::NAME)) {
            $modifiedColumns[':p' . $index++]  = '`NAME`';
        }
        if ($this->isColumnModified(AffiliateBranchPeer::PHONE)) {
            $modifiedColumns[':p' . $index++]  = '`PHONE`';
        }
        if ($this->isColumnModified(AffiliateBranchPeer::CONTACT)) {
            $modifiedColumns[':p' . $index++]  = '`CONTACT`';
        }
        if ($this->isColumnModified(AffiliateBranchPeer::CONTACTEMAIL)) {
            $modifiedColumns[':p' . $index++]  = '`CONTACTEMAIL`';
        }
        if ($this->isColumnModified(AffiliateBranchPeer::MEMO)) {
            $modifiedColumns[':p' . $index++]  = '`MEMO`';
        }

        $sql = sprintf(
            'INSERT INTO `affiliates_branch` (%s) VALUES (%s)',
