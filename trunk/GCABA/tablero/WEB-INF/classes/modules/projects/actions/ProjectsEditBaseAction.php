...
class ProjectsEditBaseAction extends BaseAction {
	...
	function prepareDocuments($mapping,$smarty,$project) {
		...
		if (class_exists('DocumentPeer') && ConfigModule::get("projects","useDocuments")) {
	...
	function prepareAdministrativeActs($mapping,$smarty,$project) {
		$administrativeActs = $project->getAdministrativeActs();	
		$smarty->assign("administrativeActs",$administrativeActs);
	}

	function prepareFinancingSources($mapping,$smarty) {
		$projectConfig = Common::getModuleConfiguration('projects');
		$sources = $projectConfig['financingSources']['element_options'];	
