...
<h1>|-if $action eq "edit"-|Editar|-else-|Crear|-/if-| Categoría de ##actors,1,Actores##</h1>
<!-- Link VOLVER -->
<!-- /Link VOLVER -->
<p>A continuación podrá |-if $action eq "edit"-|editar|-else-|crear|-/if-| una categoría de ##actors,1,Actores## del sistema.</p>
<div id="div_category">
	|-if $message eq "error"-|
...
<fieldset title="Formulario de edición de datos de una categoría">
 <legend>Ingrese los datos de la categoría</legend>
	...
	<p>			
				...
				<input type="hidden" name="do" id="do" value="actorsCategoryDoEdit" />
				<input type="submit" id="button_edit_category" name="button_edit_category" title="Aceptar" value="Aceptar" />
				<input type="button" id="button_return_category" name="button_return_category" title="Regresar" value="Regresar" onClick="location.href='Main.php?do=actorsCategoryList'" />
</p>
	</form>
</fieldset>
</div>
