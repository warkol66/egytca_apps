	...
	<fieldset>
		...
		|-/if-|
			<p class="link" |-if $content->getType()!=2 -|style="display:none"|-/if-|>
				...
				<input id="params[link]" name="params[link]" type="text" size="55" maxlength="255" value="|-$content->getLink()-|" />
			</p>
			<p class="link" |-if $content->getType()!=2-|style="display:none"|-/if-|>
				...
				<select name="params[target]" id="params[target]">
					...
					<option value="0"|-if $content->getTarget() eq 0-| selected="selected"|-/if-|>Misma ventana</option>
					<option value="1"|-if $content->getTarget() eq 1-| selected="selected"|-/if-|>Ventana nueva</option>
				</select>
			</p>
		|-foreach from=$languages item=langItem-||-if $languages|@count gt 1-|
			...
			<div id='edit_content_|-$languageCode-|'>
				<p>
					...
					<input class="emptyValidation" name="locale[|-$languageCode-|][title]" type="text" id="locale[|-$languageCode-|][title]" size="55" maxlength="255" value="|-$content->getTitle()|escape-|" />
				</p>
				<p>
				...
				<div id="pContent|-$languageCode-|" class="not_link" |-if $content->getType()==2-|style="display:none"|-/if-|>
					<p><label for="locale[|-$languageCode-|][body]">Texto</label>
						<textarea id="locale[|-$languageCode-|][body]" name="locale[|-$languageCode-|][body]" rows="10" cols="80">|-$content->getBody()|htmlentities-|</textarea>
					</p>
				</div>
			</div>
		|-/foreach-|
