...
//	}
	...
	protected function preSave(){
		...
		/**
		 * Esete es el caso de un usuario registrado en el sitio publico cambiando sus datos personales
		 */
		if ($loggedUser && get_class($loggedUser)=="RegistrationUser") {
		...
		/* Para obetener el objeto RegistrationUserInfo asociado al que se esta editando
		En acso de no tener uno se creara.
		*/
		$this->entityInfo=$this->entity->getRegistrationUserInfo();

		if(!$this->entityInfo){
			$this->entityInfo=new RegistrationUserInfo();
			$this->entity->setRegistrationUserInfo($this->entityInfo);
		}

		$this->entityInfo->fromArray($_POST["registrationUserInfo"],BasePeer::TYPE_FIELDNAME);

		$this->entity->setActive(1);

		if(!$this->entity->validate() || !$this->entityInfo->validate()){
	...
	protected function onFailure(Exception $e) {
		...
		$this->smarty->assign("registrationUserInfo",$this->entityInfo);
