...
class IPBCMS {
	...
	function IPBCMS() {
		...
		$this->loginStmt = new Statement(
			"SELECT m.member_id, m.name AS login, m.ip_address as ip_address, m.member_group_id as mgroup 
			from ".$GLOBALS["INFO"]["sql_tbl_prefix"]."members as m 
			WHERE LOWER(m.name)=? AND m.members_pass_hash = MD5(CONCAT(MD5(m.members_pass_salt),MD5(?))) LIMIT 1");

		$this->getUserIdStmt = new Statement("SELECT *
			FROM ".$GLOBALS["INFO"]["sql_tbl_prefix"]."sessions 
			WHERE id=?");
		$this->getUserStmt = new Statement("SELECT member_id, name AS login, g.g_is_supmod AS is_admin, g.g_open_close_posts AS is_moderator 
			FROM ".$GLOBALS["INFO"]["sql_tbl_prefix"]."members AS m 
				INNER JOIN ".$GLOBALS["INFO"]["sql_tbl_prefix"]."groups AS g 
				ON m.member_group_id = g.g_id 
			WHERE member_id=? LIMIT 1");

		$this->getUsersStmt = new Statement("SELECT member_id, name as login FROM ".$GLOBALS["INFO"]["sql_tbl_prefix"]."members");
