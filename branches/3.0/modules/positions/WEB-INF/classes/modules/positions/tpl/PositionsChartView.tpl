...
<div id="myTreeContainer">
		...
		<script>
			...
			function CreateTree() {
				...
				myTree.add(|-$position->getId()-|,|-$position->getParentId()-|,'|-$position->getName()-|<div class="nodeActions"><form action="Main.php" method="get" style="display:inline;"><input type="hidden" name="do" value="positionsEdit" /><input type="hidden" name="id" value="|-$position->getid()-|" /><input type="submit" name="submit_go_edit_position" value="Editar" class="buttonImageEdit" /></form><form action="Main.php" method="post" style="display:inline;"><input type="hidden" name="do" value="positionsDoDelete" /><input type="hidden" name="id" value="|-$position->getid()-|" /><input type="submit" name="submit_go_delete_position" value="Borrar" onclick="return confirm(\'Seguro que desea eliminar la posición?\')" class="buttonImageDelete" /></form></div>');
				|-/foreach-||-else-|
				|-foreach from=$positions item=position name=for_positions_nodes-|myTree.add(|-$position->getId()-|,|-$position->getParentId()-|,'<span class="data">|-$position->getName()-|</span>|-assign var=tenure value=$position->getActiveTenure()-| |-if $tenure->getObject() != NULL-|<li>|-assign var=tenureObject value=$tenure->getObject()-||-$tenureObject->getName()-| |-$tenureObject->getSurname()-|</li>|-/if-|');
				|-/foreach-||-/if-|
				myTree.UpdateTree();
			}		
			CreateTree();
