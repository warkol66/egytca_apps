...
abstract class BaseActionLogLabel extends BaseObject implements Persistent
{
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = ActionLogLabelQuery::create()
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(ActionLogLabelPeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`ID`';
        }
        if ($this->isColumnModified(ActionLogLabelPeer::ACTION)) {
            $modifiedColumns[':p' . $index++]  = '`ACTION`';
        }
        if ($this->isColumnModified(ActionLogLabelPeer::LABEL)) {
            $modifiedColumns[':p' . $index++]  = '`LABEL`';
        }
        if ($this->isColumnModified(ActionLogLabelPeer::LANGUAGE)) {
            $modifiedColumns[':p' . $index++]  = '`LANGUAGE`';
        }
        if ($this->isColumnModified(ActionLogLabelPeer::FORWARD)) {
            $modifiedColumns[':p' . $index++]  = '`FORWARD`';
        }

        $sql = sprintf(
            'INSERT INTO `actionLogs_label` (%s) VALUES (%s)',
