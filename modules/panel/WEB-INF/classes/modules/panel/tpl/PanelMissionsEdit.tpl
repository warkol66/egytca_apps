...
function deleteParticipantToMission(form){
	var fields = Form.serialize(form);
	var myAjax = new Ajax.Updater(
				{success: 'participantsList'},
				url,
...
<div id="div_mission">
	<p>Ingrese los datos del Misión</p>
		<p><a href="#" onClick="location.href='Main.php?do=panelMissionsList|-include file="FiltersRedirectUrlInclude.tpl" filters=$filters-||-if isset($page) -|&page=|-$page-||-/if-|'">Volver atras</a>
		</p>
	|-if $message eq "ok"-|
	...
	<form name="form_edit_mission" id="form_edit_mission" action="Main.php" method="post">
        ...
        <option value="|-$policyGuideline->getId()-|" |-$mission->getpolicyGuidelineId()|selected:$policyGuideline->getId()-|>|-$policyGuideline->getName()|truncate:75:"...":false-|</option> 
			...
			<p>
				...
				<input type="hidden" name="do" id="do" value="panelMissionsDoEdit" />
				<input type="submit" id="button_edit_mission" name="button_edit_mission" title="Aceptar" value="Guardar" />
				<input type="button" id="cancel" name="cancel" title="Volver al listado" value="Volver al listado" onClick="location.href='Main.php?do=panelMissionsList|-include file="FiltersRedirectUrlInclude.tpl" filters=$filters-||-if isset($page) -|&page=|-$page-||-/if-|'"/>
			</p>
		</fieldset>
	</form>
</div>
...
<div id="PartyAdding"> <span id="partieMsgField"></span> 
  <form method="post"> 
		...
		<div id="participantActor" style="position: relative;z-index:10000;">
			|-include file="CommonAutocompleterInstanceSimpleInclude.tpl" id="autocomplete_actors" label="Actor" url="Main.php?do=actorsAutocompleteListX&adminActId="|cat:$mission->getid() hiddenName="participant[actorId]" disableSubmit="addParticipant"-|
		</div>
	
		<div id="participantUser" style="display:none; position: relative;z-index:10000">
			|-include file="CommonAutocompleterInstanceSimpleInclude.tpl" id="autocomplete_users" label="Usuario" url="Main.php?do=usersAutocompleteListX&adminActId="|cat:$mission->getid() hiddenName="participant[userId]" disableSubmit="addParticipant"-|
		</div>
	<p>
      ...
      <input type="button" id="addParticipant" value="Agregar participante a mision" disabled onClick="javascript:addParticipantToMission(this.form)"/> 
    </p> 
  </form> 
	<h3>Participantes</h3>
  ...
  <ul id="partiesList">
     |-foreach from=$actualParties item=partyRel name=for_parties-| |-assign var=party value=$partyRel->getParty()-|
    <li id="partyListItem|-$party->getId()-|">|-$party->getName()-|
      <form  method="post"> 
        <input type="hidden" name="do" id="do" value="panelMissionsDoAddParticipantXAction" /> 
        <input type="hidden" name="missionId"  value="|-$mission->getId()-|" /> 
        <input type="hidden" name="partyId"  value="|-$partie->getId()-|" /> 
				<input type="button" value="Eliminar" title="Eliminar" onClick="if (confirm('¿Seguro que desea eliminar al participante del misión?')){deletePartyFromAdminAct(this.form)}; return false" class="buttonImageDelete" /> 
     </form> 
    </li> 
    |-/foreach-|
  </ul> 
...
|-if $commitments|@count gt 0-|
     |-foreach from=$commitments item=commitment name=for_commitments-|
			...
			<td>|-$commitment->getCommitment()-|</td>
			 ...
			 <form action="Main.php" method="post" style="display:inline;"> 
				...
				<form action="Main.php" method="post" style="display:inline;"> 
					...
					<input type="hidden" name="id" value="|-$commitment->getid()-|" /> 
					<input type="button" name="submit_go_delete_mission" value="Borrar" onclick="javascript: if (confirm('Seguro que desea eliminar este compromiso?')) deleteCommitment(this.form);" class="icon iconDelete" /> 
			</form>
			|-/if-|
			|-if $loginUser->isSupervisor()-|				<form action="Main.php" method="post" style="display:inline;"> 
					...
					<input type="button" name="submit_go_delete_mission" value="Borrar" onclick="javascript: if (confirm('Seguro que desea eliminar este compromiso definitivamente?')) deleteCommitment(this.form);" class="icon iconDelete" /> 
			</form>|-/if-|</td>
			 </tr>
    |-/foreach-|
|-/if-|
