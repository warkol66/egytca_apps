...
/**
 * Skeleton subclass for performing query and update operations on the 'processes_process' table.
 *
 * Process
 *
...
class ProcessPeer extends BaseProcessPeer {
	...
	*/
	function update($id,$objectiveId,$name,$description,$impact,$uniqueGoal,$goalExpirationDate,$budget,$coordinateNeed,$frequency,$finished,$notes		,$postalCode,$uniqueGoalNumeric,$goalProgress,$paramsProcess){
		$processObj = ProcessPeer::retrieveByPK($id);
		$processObj->setobjectiveId($objectiveId);
		$processObj->setname($name);
		$processObj->setdescription($description);
		$processObj->setimpact($impact);
	...
	*/
	function getByName($name,$con = null) {
		$cond = new Criteria();
		$cond->add(ProcessPeer::NAME,$name);
		$process = ProcessPeer::doSelectOne($cond,$con);
		return $process;
	}
	...
	*/
	function getAll($affiliateId = null) {
		...
		if ($affiliateId != null)  {
			...
			$cond->addJoin(ObjectivePeer::ID,ProcessPeer::OBJECTIVEID, Criteria::INNER_JOIN);
		}

		$alls = ProcessPeer::doSelect($cond);
		return $alls;
	}
	...
	*/
	function getAllPaginated($page=1,$perPage=-1,$affiliateId = null) {
		...
		$pager = new PropelPager($cond,"ProcessPeer", "doSelect",$page,$perPage);
		return $pager;
	 }
	...
	*/
	function getAllByObjective($objectiveId,$affiliateId = null) {
		...
		$cond->add(ProcessPeer::OBJECTIVEID,$objectiveId);
	...
	private function getSearchCriteria() {
		...
		if ($this->searchString)
			$criteria->add(ProcessPeer::NAME,"%".$this->searchString."%",Criteria::LIKE);

		if (!empty($this->searchDependecyId)) {
			$criteria->add(ObjectivePeer::AFFILIATEID,$this->searchDependecyId);
			$criteria->addJoin(ObjectivePeer::ID,ProcessPeer::OBJECTIVEID, Criteria::INNER_JOIN);
		}
		if (!empty($this->searchCommuneId)) {
			$criteria->add(TableroCommuneProcessPeer::COMMUNEID,$this->searchCommuneId);
			$criteria->addJoin(ProcessPeer::ID,TableroCommuneProcessPeer::PROCESSID, Criteria::INNER_JOIN);
		}
		if (!empty($this->searchRegionId)) {
			$criteria->add(TableroRegionProcessPeer::REGIONID,$this->searchRegionId);
			$criteria->addJoin(ProcessPeer::ID,TableroRegionProcessPeer::PROCESSID, Criteria::INNER_JOIN);
		}
		if (!empty($this->searchDelayed)) {
			//no finalizado y su fecha de de finalizacion es anterior a la actual.
			$criterion = $criteria->getNewCriterion(ProcessPeer::GOALEXPIRATIONDATE, date('Y-m-d')." 00:00:00", Criteria::LESS_EQUAL);
			$criterion->addAnd($criteria->getNewCriterion(ProcessPeer::FINISHED,0,Criteria::EQUAL));
		}
		if (!empty($this->searchEnded)) {
			//buscamos finalizados
			if (empty($criterion))
				$criterion = $criteria->getNewCriterion(ProcessPeer::FINISHED,1,Criteria::EQUAL);
			else
				$criterion->addOr($criteria->getNewCriterion(ProcessPeer::FINISHED,1,Criteria::EQUAL));
		}
		if (!empty($this->searchWorking)) {
			//buscamos no finalizados
			if (empty($criterion))
				$criterion = $criteria->getNewCriterion(ProcessPeer::FINISHED,0,Criteria::EQUAL);
			else
				$criterion->addOr($criteria->getNewCriterion(ProcessPeer::FINISHED,0,Criteria::EQUAL));
		}
		if (!empty($criterion))
