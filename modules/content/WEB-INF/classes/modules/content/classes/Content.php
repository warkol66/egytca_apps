...
class Content {
	...
	*/
	...
	/**
	* Devuelve los nombres de los tipos de contenido traducidos
	*/
	public function getRegionTypesTranslated() {
		$contentTypes = Content::getContentTypes();
		foreach(array_keys($contentTypes) as $key)
			$contentTypesTranslated[$key] = Common::getTranslation($contentTypes[$key],'common');
	...
	function getTypeName($type) {
		switch ($type) {
			case Content::TYPE_CONTENT: return "content";
			case Content::TYPE_SECTION: return "section";
			case Content::TYPE_LINK: return "link";
		}
	}
	...
	function getRootContents() {
		...
		$currentLanguageCode = Common::getCurrentLanguageCode();
		$result = array();
		$query = "SELECT * FROM ". $this->tablePrefix . "content_content WHERE parent = 0 AND type = " . Content::TYPE_CONTENT . " AND languageCode ='$currentLanguageCode' AND `deleted_at` IS NULL ORDER BY `order`";
		$db->query($query);
		$result = $db->recordset2Array();
		return $result;
	}
	...
	function getRootSections() {
		...
		$currentLanguageCode = Common::getCurrentLanguageCode();
		$result = array();
		$query = "SELECT * FROM ". $this->tablePrefix . "content_content WHERE parent = 0 AND type = " . Content::TYPE_SECTION . " AND languageCode ='$currentLanguageCode' AND `deleted_at` IS NULL ORDER BY `order`";
		$db->query($query);
		$result = $db->recordset2Array();
		return $result;
	}
	...
	function getRootLinks() {
		...
		$currentLanguageCode = Common::getCurrentLanguageCode();
		$result = array();
		$query = "SELECT * FROM ". $this->tablePrefix . "content_content WHERE parent = 0 AND type = " . Content::TYPE_LINK . " AND languageCode ='$currentLanguageCode' AND `deleted_at` IS NULL ORDER BY `order`";
		$db->query($query);
		$result = $db->recordset2Array();
		return $result;
	}
	...
	function getChildrenSection($id) {
		...
		$currentLanguageCode = Common::getCurrentLanguageCode();
		$result = array();
		$query = "SELECT * FROM ". $this->tablePrefix . "content_content WHERE parent = $id AND type = " . Content::TYPE_SECTION . " AND languageCode ='$currentLanguageCode' AND `deleted_at` IS NULL ORDER BY `order`";
		$db->query($query);
		$result = $db->recordset2Array();
		return $result;
	}
	...
	function getChildrenContent($id) {
		...
		$currentLanguageCode = Common::getCurrentLanguageCode();
		$result = array();
		$query = "SELECT * FROM ". $this->tablePrefix . "content_content WHERE parent = $id AND type = " . Content::TYPE_CONTENT . " AND languageCode ='$currentLanguageCode' AND `deleted_at` IS NULL ORDER BY `order`";
		$db->query($query);
		$result = $db->recordset2Array();
		return $result;
	}
	...
	function getChildrenLink($id) {
		...
		$currentLanguageCode = Common::getCurrentLanguageCode();
		$result = array();
		$query = "SELECT * FROM ". $this->tablePrefix . "content_content WHERE parent = $id AND type = " . Content::TYPE_LINK . " AND languageCode ='$currentLanguageCode' AND `deleted_at` IS NULL ORDER BY `order`";
		$db->query($query);
		$result = $db->recordset2Array();
		return $result;
	}
	...
	function addSection($id, $languageCode, $title, $titleInMenu="", $parent = null, $description = "") {
		$db = $this->db;
		$query = "INSERT INTO ". $this->tablePrefix . "content_content(id,type,parent,languageCode,title,titleInMenu,content) VALUES($id,". Content::TYPE_SECTION .",$parent,'$languageCode','$title','$titleInMenu','$description')";
		$db->query($query);
	...
	function addContent($id,$languageCode,$title,$titleInMenu="",$content,$parent,$image="",$imageOnOver="",$alt="",$link="",$order="") {
		...
		$query = "INSERT INTO ". $this->tablePrefix . "content_content(id,type,parent,languageCode,title,titleInMenu,image,imageOnOver,alt,content,link)
							VALUES($id,". Content::TYPE_CONTENT .",$parent,'$languageCode','$title','$titleInMenu','$image','$imageOnOver','$alt','$content','$link')";
		$db->query($query);
		if($db->affected_rows()>0)
			return true;
		else
	...
	function addLink($id,$languageCode,$title,$titleInMenu="",$link="",$target=0,$parent="",$image="",$imageOnOver="",$alt="",$content="",$order="") {
		...
		$query = "INSERT INTO " . $this->tablePrefix . "content_content(id,type,parent,languageCode,title,titleInMenu,image,imageOnOver,alt,content,link,target)
							VALUES ($id," . Content::TYPE_LINK . ",$parent,'$languageCode','$title','$titleInMenu','$image','$imageOnOver','$alt','$content','$link',$target)";
		$db->query($query);
		if($db->affected_rows()>0)
			return true;
		else
	...
	function getContent($id,$languageCode="") {
		...
		$db = $this->db;
		$result = array();
		$query = "SELECT * FROM ". $this->tablePrefix . "content_content WHERE id = $id AND languageCode = '$languageCode'"; // AND type = " . Content::TYPE_CONTENT;
		$db->query($query);
		$db->next_record();
		$result = $db->Record;
		return $result;
	...
	*/
	...
	function getLink($id,$languageCode="") {
		$db = $this->db;
		$result = array();
		$query = "SELECT * FROM ". $this->tablePrefix . "content_content WHERE id = $id AND type = " . Content::TYPE_LINK . " AND languageCode = '$languageCode'";
		$db->query($query);
		$db->next_record();
		$result = $db->Record;
		return $result;
	...
	*/
	...
	function getSection($id,$languageCode="") {
		...
		$db = $this->db;
		$result = array();
		$query = "SELECT * FROM ". $this->tablePrefix . "content_content WHERE id = $id AND type = " . Content::TYPE_SECTION . " AND languageCode = '$languageCode'";
		$db->query($query);
		$db->next_record();
		$result = $db->Record;
		return $result;
	...
	*/
	function updateASection($id,$languageCode,$title,$titleInMenu = "",$description = "") {
		$db = $this->db;
		$query = "UPDATE ". $this->tablePrefix . "content_content SET title = '$title', titleInMenu = '$titleInMenu', content = '$description' WHERE id = $id AND type = " . Content::TYPE_SECTION . " AND languageCode = '$languageCode'";
		$db->query($query);
		if($db->affected_rows()>0)
			return true;
		else
	...
	*/
	function updateAContent($id,$languageCode,$title,$titleInMenu="",$content,$parent,$image="",$imageOnOver="",$alt="",$link="",$order="") {
		$db = $this->db;
		$query = "UPDATE ". $this->tablePrefix . "content_content SET title = '$title', titleInMenu = '$titleInMenu', content = '$content', image = '$image', imageOnOver = '$imageOnOver', alt = '$alt', link = '$link',parent = $parent WHERE id = $id AND type = " . Content::TYPE_CONTENT . " AND languageCode = '$languageCode'";
		$db->query($query);
		if($db->affected_rows()>0)
			return true;
		else
	...
	*/
	...
	function updateALink($id,$languageCode,$title,$titleInMenu="",$link,$target,$parent,$image="",$imageOnOver="",$alt="",$content="",$order="") {
		$db = $this->db;
		$query = "UPDATE ". $this->tablePrefix . "content_content SET title = '$title', titleInMenu = '$titleInMenu', content = '$content', image = '$image', imageOnOver = '$imageOnOver', alt = '$alt', link = '$link', parent = $parent, target = $target WHERE id = $id AND type = " . Content::TYPE_LINK . " AND languageCode = '$languageCode'";
		$db->query($query);
		if($db->affected_rows()>0)
			return true;
		else
	...
	function deleteSection($id) {
		...
		if ($this->sectionHasChildren($id)) {
			...
			foreach ($children as $child) {
				if ($child['type'] == Content::TYPE_CONTENT)
					$this->deleteContent($child['id']);
				if ($child['type'] == Content::TYPE_LINK)
					$this->deleteLink($child['id']);
				if ($child['type'] == Content::TYPE_SECTION)
	...
	*/
	...
	function getMenuRecursive($sectionId=0,$depth,$currentDepth) {
		...
		for ($i = 0; $i < count($rootElements) ; $i++) {
			if ($rootElements[$i]['type'] == Content::TYPE_SECTION) {
	...
	*/
	function getMenu($content,$backToParent,$depth) {
		...
		for ($i = 0; $i < count($rootElements['menu']) ; $i++) {
			if ($rootElements['menu'][$i]['type'] == Content::TYPE_SECTION) {
	...
	function getSections() {
		...
		$result = array();
		$languageCode = Common::getCurrentLanguageCode();
		$query = "SELECT * FROM ". $this->tablePrefix . "content_content WHERE type = " . Content::TYPE_SECTION . " AND `languageCode` = '$languageCode' AND `deleted_at` IS NULL ORDER BY `order`";
		$db->query($query);
		$result = $db->recordset2Array();
		return $result;
	}
