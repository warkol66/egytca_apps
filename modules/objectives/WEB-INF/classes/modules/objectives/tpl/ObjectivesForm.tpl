    ...
    |-/if-|
    	...
    	|-/if-|
     	...
     	|-if ($loginUser neq "" and $loginUser->isAdmin() && $moduleConfig.useDependencies.value == "YES")-|
      	...
      	|-/if-|  
     	|-if ($strategicObjectives|@count) gt 0-|
     		<p>
      			...
      			<select id="strategicObjectiveId" name="paramsObjective[strategicObjectiveId]" title="##objectives,2,Objetivo Etratégico##" |-$action|disabled-|> 
					<option value="">Seleccione objetivo estratégico</option>
    	...
    	<p>
      		...
      		<select id="paramsObjective[responsibleCode]" name="paramsObjective[responsibleCode]" title="Responsable" |-$action|disabled-|> 
				...
				|-foreach from=$positions item=position name=for_positions-|
       				<option value="|-$position->getCode()-|" |-$objective->getResponsibleCode()|selected:$position->getCode()-|>|-section name=space loop=$position->getLevel()-|&nbsp;|-/section-||-$position->getName()-| |-if get_class($position->getActiveTenureName()) eq "PositionTenure"-||-assign var=tenure value=$position->getActiveTenureName()-||-if $tenure->getName() ne ''-| &#8212; |-$tenure->getName()-||-/if-||-else-||-assign var=userInfo value=$position->getActiveTenureName()-||-if $userInfo->getName() ne '' || $userInfo->getSurname() ne ''-| &#8212; |-/if-||-$userInfo->getName()-| |-$userInfo->getSurname()-||-/if-|</option> 
				|-/foreach-|
      		</select> 
     	</p> 
    	<p>
