...
class ObjectivesStrategicObjectivesPlotGanttAction extends BaseAction {


	// ----- Constructor ---------------------------------------------------- //

	function ObjectivesStrategicObjectivesPlotGanttAction() {
		;
	}
	...
	*/
	function execute($mapping, $form, &$request, &$response) {
		...
		if (isset($_GET['objectiveId'])) {
			...
			$objective = StrategicObjectivePeer::get($_GET['objectiveId']);
...
//				$graph->SetWeekStart(1);
				...
				for ($i = 0; $i < count($projects) ; $i++) {
					...
					$activity->SetCSIMTarget("Main.php?do=objectivesMilestonesNav&projectId=".$project->getId());
					$activity->SetCSIMAlt(htmlspecialchars($project->getName()));
					$activity->title->SetCSIMTarget("Main.php?do=objectivesMilestonesNav&projectId=".$project->getId());
