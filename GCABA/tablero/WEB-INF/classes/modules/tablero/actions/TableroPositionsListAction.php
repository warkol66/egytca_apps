...
class TableroPositionsListAction extends BaseAction {


	// ----- Constructor ---------------------------------------------------- //

	function TableroPositionsListAction() {
		;
	}
	...
	*/
	function execute($mapping, $form, &$request, &$response) {
		...
		if (isset($_GET['filters'])) {
			...
			foreach(array_keys($positionPeer->filterConditions) as $filterKey) {

				if (isset($filters[$filterKey])) {
					//obtengo el metodo de la condicion de filtrado a ejecutar
					$filterMethod = $positionPeer->filterConditions[$filterKey];
					//ejecuto el metodo que agrega la condicion
					$positionPeer->$filterMethod($filters[$filterKey]);
