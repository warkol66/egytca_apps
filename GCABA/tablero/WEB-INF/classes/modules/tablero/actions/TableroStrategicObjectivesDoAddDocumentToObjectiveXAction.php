...
class TableroStrategicObjectivesDoAddDocumentToObjectiveXAction extends BaseAction {


	// ----- Constructor ---------------------------------------------------- //

	function TableroStrategicObjectivesDoAddDocumentToObjectiveXAction() {
		;
	}
	...
	function execute($mapping, $form, &$request, &$response) {
		...
		if ( !empty($_POST["objectiveId"]) && !(empty($_POST["documentId"])) ) {
			...
			if (!empty($objective) && !empty($document)) {
				...
				$relation = new TableroDocumentObjective();
