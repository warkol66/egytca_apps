...
class TableroStrategicObjectivesBarNavAction extends BaseAction {


	// ----- Constructor ---------------------------------------------------- //

	function TableroStrategicObjectivesBarNavAction() {
		;
	}
	...
	*/
	function execute($mapping, $form, &$request, &$response) {
		...
		if (!empty($_GET["page"])) {
			$pager = TableroStrategicObjectivePeer::getAllPaginated($_GET["page"],-1,$dependencyId);
			$objectives = $pager->getResult();
		}
		else
			$objectives = TableroStrategicObjectivePeer::getAll($dependencyId);
		...
		$graph->subtitle->Set(Common::getTranslation(TableroStrategicObjectivePeer::ITEM_NAME,'tablero'));
