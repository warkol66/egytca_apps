    ...
    <li id="partyListItem|-$party->getId()-|">
      <form  method="post" id="partyListItemForm|-$party->getId()-|"> 
        ...
        <input type="hidden" name="partyId"  value="|-$party->getId()-|" /> 
				<input type="button" value="Eliminar" title="Eliminar" onClick="if (confirm('¿Seguro que desea eliminar al participante de la misión?')){deleteParticipantFromMission(this.form)}; return false" class="icon iconDelete" /></form> 
