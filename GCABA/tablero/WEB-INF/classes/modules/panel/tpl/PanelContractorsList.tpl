...
<div id="div_contractors"> 
	...
	<table id="tabla-contractors" class='tableTdBorders' cellpadding='5' cellspacing='0' width='100%'> 
		...
		<tr>
			...
			<tr>
				 <th colspan="6" class="thFillTitle"><div class="rightLink"><a href="Main.php?do=panelContractorsEdit|-include file="FiltersRedirectUrlInclude.tpl" filters=$filters-||-if isset($pager) && ($pager->getPage() ne 1)-|&page=|-$pager->getPage()-||-/if-|" class="addLink">Agregar Contratista</a></div></th>
			</tr>
	...
	<!--		<td>|-$contractor->getid()-|</td> -->
			...
			<td nowrap> <form action="Main.php" method="get" style="display:inline;"> 
					...
					<input type="submit" name="submit_go_edit_contractor" value="Editar" class="icon iconEdit" /> 
				</form>
				...
				<form action="Main.php" method="post" style="display:inline;"> 
					<input type="hidden" name="do" value="panelContractorsDoDelete" /> 
						...
						|-if isset($pager) && ($pager->getPage() ne 1)-| <input type="hidden" name="page" id="page" value="|-$pager->getPage()-|" />|-/if-|
					<input type="hidden" name="id" value="|-$contractor->getid()-|" /> 
					<input type="submit" name="submit_go_delete_contractor" value="Borrar" onclick="return confirm('Seguro que desea eliminar el contratista?')" class="icon iconDelete" /> 
			</form>
			|-/if-|
			|-if $loginUser->isSupervisor()-|				<form action="Main.php" method="post" style="display:inline;"> 
					...
					<input type="submit" name="submit_go_delete_contractor" value="Borrar" onclick="return confirm('Seguro que desea eliminar el contratista definitivamente?')" class="icon iconDelete" /> 
			</form>|-/if-|</td> 
		</tr> 
		|-/foreach-|
		...
		|-/if-|
			<tr>
				 <th colspan="6" class="thFillTitle">|-if $contractors|@count gt 5-|<div class="rightLink"><a href="Main.php?do=panelContractorsEdit|-include file="FiltersRedirectUrlInclude.tpl" filters=$filters-||-if isset($pager) && ($pager->getPage() ne 1)-|&page=|-$pager->getPage()-||-/if-|" class="addLink">Agregar Contratista</a></div>|-/if-|</th>
			</tr>
		|-/if-|
		</tbody> 
		 </table> 
