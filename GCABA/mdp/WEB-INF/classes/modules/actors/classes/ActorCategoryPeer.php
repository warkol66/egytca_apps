...
/**
 * Skeleton subclass for performing query and update operations on the 'actors_category' table.
 *
 ...
 */
class ActorCategoryPeer extends BaseActorCategoryPeer {
	...
	*/
	function get($id){
		$categoryObj = ActorCategoryQuery::create()->findPk($id);
		return $categoryObj;
	}
	/**
	...
	*/
	function getAll(){
		$categories = ActorCategoryQuery::create()->orderByBranch()->where('ActorCategory.Scope >= ?', 0)->find();
		return $categories;
	}
	...
	*/
	function getAllPaginated($page=1,$perPage=-1){
		...
		$criteria->add(ActorCategoryPeer::SCOPE, 0, Criteria::GREATER_EQUAL);
		$pager = new PropelPager($criteria,"ActorCategoryPeer", "doSelect",$page,$perPage);
		return $pager;
	 }
  ...
  private function getCriteria(){
		$criteria = ActorCategoryQuery::create()->orderByScope()->orderByBranch();
		$criteria->setIgnoreCase(true);
		$criteria->add(ActorCategoryPeer::SCOPE, 0, Criteria::GREATER_EQUAL);
	
		if ($this->searchString)
	    $criteria->add(ActorCategoryPeer::NAME,"%".$this->searchString."%",Criteria::LIKE);
