  <ul id="partiesList" class="iconOptionsList">
    ...
    <li id="partyListItem|-$party->getId()-|">
      <form  method="post"> 
        <input type="hidden" name="do" id="do" value="servicesSchedulesSubscriptionsDoDeleteUserX" /> 
        ...
        <input type="hidden" name="partyId"  value="|-$party->getId()-|" /> 
				<input type="button" value="Eliminar" title="Eliminar" onClick="if (confirm('¿Seguro que desea eliminar al destinatario de la suscripción?')){deleteUserFromScheduleSubscription(this.form)}; return false" class="icon iconDelete" /></form> 
