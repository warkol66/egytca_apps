...
<form method='post'>
		...
		<input type="text" name="params[name]" id="name" value='|-$category->getName()-|' size="50" />
		</p>
		<p>
			<label for="category_responsible]">Funcionario a cargo</label>
			<input name="params[responsible]" type="text" id="category_responsible" title="Responsable a cargo" value="|-$category->getResponsible()-|" size="80" />
		</p>

	|-if $category->isChildren()-|
		<p><label for="params[parentId]">Dependencia de </label>
		<select name="params[parentId]">
	...
	|-else-|
		<input type="hidden" name="params[parentId]" value="|-$category->getParentId()-|" id="parentId">
	|-/if-|
		<p>
		|-include file="HiddenInputsInclude.tpl" action="$action" filters="$filters" page="$page"-|
		|-if !$category->isNew()-|<input type="hidden" name="id" id="id" value="|-$category->getId()-|" />|-/if-|
