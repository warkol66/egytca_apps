{
	...
	"allDay": false,
	"className": "|-assign var="axis" value=$event->getCalendarAxis()-||-if $axis-||-$axis->getCssClass()-||-else-|gris|-/if-||-if $event->getEndDate() gt '0000-00-00' && $event->getEndDate() lte $smarty.now|date_format:'%Y-%m-%d'-| evAnterior|-else if $event->getStartDate() lt $smarty.now|date_format:'%Y-%m-%d'-| evAnterior|-else-| evReflex|-/if-|",
