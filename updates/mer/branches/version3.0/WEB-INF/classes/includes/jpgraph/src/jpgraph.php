...
//user
//
//Comentado por problemas con la nueva version de phpMVC
...
class Legend {
    ...
    public $balloon = false;
    public $balloon_width = 100;
//---------------
// CONSTRUCTOR
    function Legend() {
	// Empty
	...
	foreach($this->txtcol as $p) {
		...
		if( $p[2]->GetType() != MARK_IMG ) {
		    ...
		    if( $p[2]->GetType() == MARK_FILLEDCIRCLE || $p[2]->GetType() == MARK_CIRCLE ) {
		        ...
		        $yCircle = max($p[2]->GetWidth()*2,10);
			$p[2]->Stroke($aImg,round($x1+30+$this->xmargin),$y1+$yCircle);
		    }
		    else {
	    ...
	    $aImg->SetTextAlign("left","top");
			if ($this->balloon)
      	$aImg->StrokeText(round($x1+$this->mark_abs_hsize+$this->xmargin),$y1+$yCircle,$p[0]);
      else
	    ...
	    if( $i >= $this->layout_n ) {
		$x1 = $xp+$this->mark_abs_hsize+$this->xlmargin;
		if( empty($p[2]) || ( $p[2]->GetType() != MARK_FILLEDCIRCLE && $p[2]->GetType() != MARK_CIRCLE ) ) {
			$y1 += $rowheight[$row++];
		}
		else 
			$y1 += $yCircle;
