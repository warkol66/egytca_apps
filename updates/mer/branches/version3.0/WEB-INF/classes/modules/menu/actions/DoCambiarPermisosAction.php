...
* @public
*/
class DoCambiarPermisosAction extends BaseAction {


	// ----- Constructor ---------------------------------------------------- //

	function DoCambiarPermisosAction() {
		;
	}
	...
	*/
    ...
    BaseAction::execute($mapping, $form, $request, $response);
		...
		{
			$sql  = "SELECT PERMISO FROM MOD_TIPO_USUARIO WHERE id_tipo_usuario='".$_POST['id_tipo_usuario']."'";
			$db->query($sql);
			list($permiso) = $db->convarray('PERMISO');
		}
		if (!empty($_POST['agregar']))
		{
			$permisosObject->borrarTodos();
			foreach ($_POST['agregar'] as $action=>$value)
			{
				$permisosObject->guardar($action,$permiso);
			}
		}
