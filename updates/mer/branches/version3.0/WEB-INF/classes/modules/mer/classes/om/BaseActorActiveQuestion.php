...
/**
 * Base class that represents a row from the 'MER_actorActiveQuestion' table.
 *
 * 
 *
 * @package    propel.generator.mer.classes.om
 */
abstract class BaseActorActiveQuestion extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'ActorActiveQuestionPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        ActorActiveQuestionPeer
	 */
	 ...
	 * @return     ActorActiveQuestion The current object (for fluent API support)
	 */
	public function setActorid($v)
	{
		...
		if ($this->actorid !== $v) {
			$this->actorid = $v;
			$this->modifiedColumns[] = ActorActiveQuestionPeer::ACTORID;
		}
	...
	public function setQuestionid($v)
	{
		...
		if ($this->questionid !== $v) {
			$this->questionid = $v;
			$this->modifiedColumns[] = ActorActiveQuestionPeer::QUESTIONID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = ActorActiveQuestionPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating ActorActiveQuestion object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = ActorActiveQuestionPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				ActorActiveQuestionPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedRows += ActorActiveQuestionPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = ActorActiveQuestionPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = ActorActiveQuestionPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['ActorActiveQuestion'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['ActorActiveQuestion'][serialize($this->getPrimaryKey())] = true;
		$keys = ActorActiveQuestionPeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ActorActiveQuestionPeer::ACTORID)) $criteria->add(ActorActiveQuestionPeer::ACTORID, $this->actorid);
		if ($this->isColumnModified(ActorActiveQuestionPeer::QUESTIONID)) $criteria->add(ActorActiveQuestionPeer::QUESTIONID, $this->questionid);
	...
	{
		$criteria = new Criteria(ActorActiveQuestionPeer::DATABASE_NAME);
		$criteria->add(ActorActiveQuestionPeer::ACTORID, $this->actorid);
		$criteria->add(ActorActiveQuestionPeer::QUESTIONID, $this->questionid);
	...
	{
		return (null === $this->getActorid()) && (null === $this->getQuestionid());
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of ActorActiveQuestion (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     ActorActiveQuestion Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     ActorActiveQuestionPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new ActorActiveQuestionPeer();
		}
		return self::$peer;
	}
	...
	public function getActor(PropelPDO $con = null)
	{
		if ($this->aActor === null && ($this->actorid !== null)) {
			...
			/* The following can be used additionally to
				...
				$this->aActor->addActorActiveQuestions($this);
			 */
		}
		return $this->aActor;
	}
	...
	public function getQuestion(PropelPDO $con = null)
	{
		if ($this->aQuestion === null && ($this->questionid !== null)) {
			...
			/* The following can be used additionally to
				...
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aQuestion->addActorActiveQuestions($this);
			 */
		}
	...
	{
		return (string) $this->exportTo(ActorActiveQuestionPeer::DEFAULT_STRING_FORMAT);
	}
