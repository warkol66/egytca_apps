...
/**
 * Base class that represents a row from the 'MER_formAnswer' table.
 *
 * 
 *
...
abstract class BaseAnswer extends BaseObject  implements Persistent
{
	...
	public function setActorid($v)
	{
		...
		if ($this->actorid !== $v) {
			$this->actorid = $v;
			$this->modifiedColumns[] = AnswerPeer::ACTORID;
		}
	...
	public function setQuestionid($v)
	{
		...
		if ($this->questionid !== $v) {
			$this->questionid = $v;
			$this->modifiedColumns[] = AnswerPeer::QUESTIONID;
		}
	...
	public function setJudgement($v)
	{
		...
		if ($this->judgement !== $v) {
			$this->judgement = $v;
			$this->modifiedColumns[] = AnswerPeer::JUDGEMENT;
		}
	...
	public function setOld($v)
	{
		...
		if ($this->old !== $v) {
			$this->old = $v;
			$this->modifiedColumns[] = AnswerPeer::OLD;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 6; // 6 = AnswerPeer::NUM_HYDRATE_COLUMNS.
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(AnswerPeer::ID) ) {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AnswerPeer::ACTORID)) $criteria->add(AnswerPeer::ACTORID, $this->actorid);
		if ($this->isColumnModified(AnswerPeer::QUESTIONID)) $criteria->add(AnswerPeer::QUESTIONID, $this->questionid);
		if ($this->isColumnModified(AnswerPeer::ANSWER)) $criteria->add(AnswerPeer::ANSWER, $this->answer);
		if ($this->isColumnModified(AnswerPeer::JUDGEMENT)) $criteria->add(AnswerPeer::JUDGEMENT, $this->judgement);
		if ($this->isColumnModified(AnswerPeer::OLD)) $criteria->add(AnswerPeer::OLD, $this->old);
