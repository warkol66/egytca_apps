...
/**
 * Base class that represents a row from the 'MER_form' table.
 *
 * 
 *
...
abstract class BaseForm extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the rootsectionid field.
	 * @var        int
	 */
	protected $rootsectionid;
	 ...
	 * Is this form a relationship form (between cities)?
	 * @return     boolean
	 */
	...
	/**
	 * Get the [rootsectionid] column value.
	 * 
	 * @return     int
	 */
	...
	public function setRelationship($v)
	{
		...
		if ($this->relationship !== $v || $this->isNew()) {
			$this->relationship = $v;
			$this->modifiedColumns[] = FormPeer::RELATIONSHIP;
		}
	...
	/**
	 * Set the value of [rootsectionid] column.
	 * 
	 * @param      int $v new value
	 * @return     Form The current object (for fluent API support)
	 */
	public function setRootsectionid($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->rootsectionid !== $v) {
			$this->rootsectionid = $v;
			$this->modifiedColumns[] = FormPeer::ROOTSECTIONID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->rootsectionid = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->resetModified();
			...
			return $startcol + 4; // 4 = FormPeer::NUM_HYDRATE_COLUMNS.
	...
	public function ensureConsistency()
	{
		...
		if ($this->aFormSection !== null && $this->rootsectionid !== $this->aFormSection->getId()) {
			$this->aFormSection = null;
		}
	} // ensureConsistency
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(FormPeer::ID) ) {
	...
	{
		if (isset($alreadyDumpedObjects['Form'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Form'][$this->getPrimaryKey()] = true;
		$keys = FormPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
			$keys[2] => $this->getRelationship(),
			$keys[3] => $this->getRootsectionid(),
		);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setRootsectionid($arr[$keys[3]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FormPeer::RELATIONSHIP)) $criteria->add(FormPeer::RELATIONSHIP, $this->relationship);
		if ($this->isColumnModified(FormPeer::ROOTSECTIONID)) $criteria->add(FormPeer::ROOTSECTIONID, $this->rootsectionid);
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setRootsectionid($this->getRootsectionid());
		if ($makeNew) {
			$copyObj->setNew(true);
	...
	{
		if ($v === null) {
			$this->setRootsectionid(NULL);
		} else {
			$this->setRootsectionid($v->getId());
		}
	...
	{
		if ($this->aFormSection === null && ($this->rootsectionid !== null)) {
			$this->aFormSection = FormSectionQuery::create()->findPk($this->rootsectionid, $con);
			/* The following can be used additionally to
				guarantee the related object contains a reference
				to this object.  This level of coupling may, however, be
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aFormSection->addForms($this);
			 */
		}
