			...
			|-if isset($unifiedLogin)-|
				...
				<p>Tipo de Usuario
					 <select name="selectLoginMode" id="selectLoginMode" onChange="javascript:selectLoginAction(this.form)"> 
						...
						<option value="clientUser" |-if $cookieSelection eq 'clientUser'-|selected="selected"|-/if-|>Cliente</option> 
					</select> 
				 </p> 
				|-/if-|
				<input type="hidden" name="do" value="|-if $cookieSelection eq 'clientUser'-|clientsUsersDoLogin|-else-|usersDoLogin|-/if-|" id="loginFormDo" />
			|-else-|
