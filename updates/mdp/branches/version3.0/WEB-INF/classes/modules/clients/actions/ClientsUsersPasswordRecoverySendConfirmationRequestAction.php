<?php
/**
 * ClientsUsersPasswordRecoverySendConfirmationRequestAction
 *
 * @package clients
 */
...
class ClientsUsersPasswordRecoverySendConfirmationRequestAction extends BaseAction {

	function ClientsUsersPasswordRecoverySendConfirmationRequestAction() {
		;
	}
			...
			if (Common::validateCaptcha($_POST['securityCode'])) {
				...
				if ( !empty($user)) {
					if (!$user->recoveryRequestAlredyMade()) {
						...
						$body = $smarty->fetch("ClientsUsersPasswordRecoveryConfirmationRequestMail.tpl");
