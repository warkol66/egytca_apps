<?php
/** 
 * ClientsUsersPasswordDoChangeAction
 *
 * @package client 
 */

class ClientsUsersPasswordDoChangeAction extends BaseAction {

	function ClientsUsersPasswordDoChangeAction() {
		;
	}
		...
		if (($currentPass == $user->getPassword()) && ($_POST["pass"] == $_POST["pass2"]) ) {
			...
			if ($_POST['currentPass'] == $_POST["pass"]){
				...
				if ($userPeer->updatePass($user->getId(),$_POST["pass"],$_POST['mailAddress'],$_POST['timezone']) ) {
					...
					header("Location: Main.php?do=clientsUsersPasswordChange&message=errorUpdate");
					exit;
				}
			}
		}
			...
			header("Location: Main.php?do=clientsUsersPasswordChange&message=wrongPassword");
			exit;
		}
